...
# [2/3] 백엔드 (venv Python)
Write-Host "[2/3] Starting Backend Server (port 8080)..." -ForegroundColor Yellow
$env:PYTHONUTF8 = "1"
Start-Process cmd -ArgumentList "/k cd /d `"$BASE\situation-backend`" && `"$BASE\venv\Scripts\python.exe`" -m uvicorn session.main:app --reload --host 0.0.0.0 --port 8080" -WindowStyle Normal
...
Write-Host " Frontend Mobile: http://92.168.219.170:3000 (스마트폰/태블릿 접속)"
Write-Host "------------------------------------------------------"
Write-Host ""
Pause
