            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px', overflow: 'auto', minHeight: 0 }}>
                    {selectedTableId && selectedStage ? (
                        <>
                            ...
                            {/* ── 합계 + 세션 액션 바 ── */}
                            {selectedSession && (
                                <div style={{ background: 'var(--surface)', borderRadius: '10px', border: '1px solid var(--border)', padding: '6px 8px', display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'space-between' }}>
                                    {/* 결제 요약 */}
                                    <div style={{ flex: 1, display: 'flex', gap: '6px', minWidth: 0 }}>
                                        <div style={{ whiteSpace: 'nowrap', minWidth: 0 }}>
                                            <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', fontWeight: '700', whiteSpace: 'nowrap' }}>결제완료</div>
                                            <div style={{ fontSize: '0.82rem', fontWeight: '900', color: '#10b981', whiteSpace: 'nowrap' }}>{paidTotal.toLocaleString()}원</div>
                                        </div>
                                        <div style={{ whiteSpace: 'nowrap', minWidth: 0 }}>
                                            <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', fontWeight: '700', whiteSpace: 'nowrap' }}>미결제</div>
                                            <div style={{ fontSize: '0.82rem', fontWeight: '900', color: unpaidTotal > 0 ? '#ef4444' : '#10b981', whiteSpace: 'nowrap' }}>
                                                {unpaidTotal > 0 ? `${unpaidTotal.toLocaleString()}원` : activeOrders.length === 0 ? '없음' : '완료'}
                                            </div>
                                        </div>
                                        <div style={{ whiteSpace: 'nowrap', minWidth: 0 }}>
                                            <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', fontWeight: '700', whiteSpace: 'nowrap' }}>합계</div>
                                            <div style={{ fontSize: '0.82rem', fontWeight: '900', color: 'var(--text-main)', whiteSpace: 'nowrap' }}>{sessionTotal.toLocaleString()}원</div>
                                        </div>
                                    </div>
 
                                    {/* 액션 버튼 */}
                                    {isPending ? (
                                        <button onClick={() => handleOpenSession(selectedSession.table_id)} style={{ background: '#f97316', border: 'none', color: 'white', padding: '5px 8px', borderRadius: '5px', fontWeight: '700', fontSize: '0.72rem', cursor: 'pointer', whiteSpace: 'nowrap' }}>개시 승인</button>
                                    ) : (
                                        <div style={{ display: 'flex', gap: '3px', flexShrink: 0 }}>
                                            <button onClick={() => handleResetSession(selectedSession.session_id)} style={{ background: 'transparent', border: '1px solid #fca5a5', color: '#ef4444', padding: '5px 6px', borderRadius: '5px', fontWeight: '600', fontSize: '0.72rem', cursor: 'pointer', whiteSpace: 'nowrap' }}>초기화</button>
                                            {unpaidTotal === 0 ? (
                                                <button 
                                                ...
                                                <button onClick={() => setSelectedSessionForPay(selectedSession)} style={{ background: 'var(--primary)', border: 'none', color: 'white', padding: '5px 10px', borderRadius: '5px', fontWeight: '700', fontSize: '0.72rem', cursor: 'pointer', whiteSpace: 'nowrap' }}>전체결제</button>
                                            )}
                                        </div>
                                    )}
                                </div>
